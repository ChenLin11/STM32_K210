...
//	TIM_SetCompare1(TIM3,180);//每+10,舵机转角+90°
//	delay_ms(1000);
	TIM_SetCompare1(TIM3,170);//默认为关闭状态
	delay_ms(1000);
	while(1)
	{ 
		LCD_Fill(30,50,240,66,WHITE);//清除显示	 
		...
		{						   
			...
			{				 
				case WKUP_PRES:	//key_up键
					sendMessage("AAA");//用于添加人脸信息
					LED1 = !LED1;
					break;
				case KEY1_PRES:	//key1键
					sendMessage("DDD");//用于删除人脸信息
					LED0 = !LED0;
					break;
				case KEY0_PRES:
					sendMessage("RRR");//查看入侵人脸图像
					break;
				case KEY2_PRES:
					sendMessage("GGG");//删除入侵人脸图像
					break;
			}
		}else delay_ms(10); 
	} 	
